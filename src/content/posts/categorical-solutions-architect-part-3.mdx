...
A **representable functor**[^2] is one where there exists a "universal element" that generates all other elements. In architecture:
...
Consumer-driven contracts[^3] (CDC) embody the Yoneda perspective perfectly.
...
The service is characterized by the **union of consumer expectations**. If all consumers are satisfied, the service is correct—for the interactions those consumers exercise—regardless of how it's implemented.

<InfoBox title="Yoneda Says" type="tip">

The service *is* the collection of all consumer views. Any implementation that satisfies all consumer contracts is, categorically speaking, the same service—relative to those contracts. One honest caveat: Yoneda requires agreement on *all* morphisms, naturally, while a contract suite only samples $\text{Hom}(-, A)$. Two implementations that pass every existing contract can still differ on interactions no current consumer exercises.
...
In category theory, a **presheaf**[^4] on a category $\mathcal{C}$ is a functor $\mathcal{C}^{op} \to \text{Set}$. This assigns to each object a set and to each morphism a function going the "opposite" direction.
...
Within any single view, the presheaf structure demands **functoriality**: observing along a composed morphism must equal composing the observations—$F(g \circ f) = F(f) \circ F(g)$. That keeps each view internally coherent, but it does *not* force different stakeholder views to agree with each other.
...
That stronger property—independent views agreeing on their overlaps and gluing into one global picture—is the **sheaf** condition. When it holds, you don't just have a presheaf, you have a sheaf. Presheaves famously need not satisfy it, which is exactly why stakeholder views drift apart unless you actively make them glue.

---

## Interface Segregation Is Yoneda

The Interface Segregation Principle[^5] (ISP) from SOLID says:
...
Everything else—runtime, memory, timeout, VPC configuration—is implementation detail. Two Lambdas with the same interface *and the same observable behavior on every event* are interchangeable.
