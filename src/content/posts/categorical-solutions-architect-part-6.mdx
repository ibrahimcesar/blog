...
```typescript
// For any operation f (e.g., applying a discount):
// F(f) = legacyApplyDiscount — the operation as legacy runs it
// G(f) = newApplyDiscount — the operation as the new system runs it
// α = transformResult — the migration map

// Path 1: legacy operation, then migrate
const path1 = transformResult(legacyApplyDiscount(order));

// Path 2: migrate, then new operation
const path2 = newApplyDiscount(transformResult(order));

// Naturality requires: path1 === path2
```
...
Feature flags control which functor handles each request:
...
```typescript
// Consistent routing: same user stays on the same path
// — a precondition for sanity, not naturality itself
...
One caveat: the flag itself is not the natural transformation. It supplies no components $\alpha_A: F(A) \to G(A)$ — it just routes each input to $F$ or $G$. Pinning a user to one path is session stickiness. What makes mixed operation coherent is the underlying migration map $\alpha$: legacy results must migrate to what the new system would have produced.

<InfoBox title="Gradual Rollout" type="tip">

A gradual rollout is a time-indexed family of *functors* $F_t$ — the per-request routing in effect at time $t$ — with $F_0 = F$ and $F_1 = G$, connected by natural transformations between successive stages. The rollout moves through intermediate functors, not through "partial" transformations: any $\alpha: F \Rightarrow G$ is already the full transformation.
...
Step Functions InputPath/OutputPath/ResultPath are data-mapping morphisms:
...
The paths define how data transforms between states—each is a single data-mapping function, chained by ordinary function composition. No functor pair, no naturality square — just morphisms.
...
```
F(Request) --F(handle)--> F(Response)
     |                         |
     α_Request                 α_Response
     |                         |
     v                         v
G(Request) --G(handle)--> G(Response)
```

Blue is $F$, green is $G$; `handle` is request processing. The square must commute: handle in blue then migrate the response, or migrate the request then handle in green — same result.
...
// Consistent routing: same user always hits same environment
// (until we change the percentage) — a precondition, not naturality.
// Naturality proper: run blue then migrate === migrate then run green
```
...
[^5]: **Horizontal composition** (also called the Godement product) is more subtle. Given $\alpha: F \Rightarrow G$ (between $\mathcal{C}$ and $\mathcal{D}$) and $\beta: H \Rightarrow K$ (between $\mathcal{D}$ and $\mathcal{E}$), you get $\beta * \alpha: H \circ F \Rightarrow K \circ G$. The components are $(\beta * \alpha)_A = \beta_{G(A)} \circ H(\alpha_A) = K(\alpha_A) \circ \beta_{F(A)}$ (these are equal by naturality of $\beta$). This corresponds to migrating multiple systems in parallel — the order doesn't matter because the transformations are independent. **Whiskering** is the special case where one of the two transformations is an identity — composing a natural transformation with a functor on one side, written $H\alpha$ or $\alpha H$.

[^6]: A **natural isomorphism** is a natural transformation where every component $\alpha_A: F(A) \to G(A)$ is an isomorphism (has an inverse). This means the two functors $F$ and $G$ are "essentially the same" — they capture the same structure, just represented differently. Natural isomorphisms are written $F \cong G$. Two categories are *equivalent* when there are functors $F: \mathcal{C} \to \mathcal{D}$ and $G: \mathcal{D} \to \mathcal{C}$ whose composites are naturally isomorphic to the identities ($G \circ F \cong \mathrm{Id}_{\mathcal{C}}$ and $F \circ G \cong \mathrm{Id}_{\mathcal{D}}$) — the subject of the next post.
