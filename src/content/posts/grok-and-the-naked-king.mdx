...
[^2]: **Constitutional AI** was introduced by Anthropic in their [2022 paper](https://arxiv.org/abs/2212.08073). The approach uses a set of written principles (the "constitution") to guide AI behavior, with the model critiquing and revising its own outputs based on these principles. The method significantly reduces reliance on human feedback — AI feedback costs less than $0.01 per prompt compared to $1-10+ for human preference data. The elegance of the approach masks its central weakness: the constitution itself is written by the company, interpreted by the company, and modified by the company when convenient. It's not alignment with humanity; it's alignment with whoever drafts the document.
