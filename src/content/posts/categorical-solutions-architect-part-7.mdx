...
class CheckoutModule {
  constructor(private orderRepo: OrderRepo, private paymentRepo: PaymentRepo) {}
...
An **adjoint equivalence** is an equivalence where the functors form an adjunction (which we'll cover in Part 11). It's not a stronger relationship between the categories—every equivalence can be promoted to an adjoint equivalence using the same two functors—but a convenient normalization of one.[^11]
...
```typescript
// Quasi-inverse pair—each undoes the other up to isomorphism
declare const decompose: (monolith: Monolith) => Microservices;
declare const compose: (microservices: Microservices) => Monolith;

// In an equivalence, each functor is both left and right adjoint
// to the other—no free ⊣ forgetful asymmetry here
```
...
**Equivalent for**: Known access patterns[^12]
