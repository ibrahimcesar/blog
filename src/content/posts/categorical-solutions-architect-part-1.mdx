...
If the order of updates matters, then—writing $X; Y$ for "X then Y" in diagrammatic order, i.e. $Y \circ X$:

$$
(X; Y) \neq (Y; X)
$$

The inequality itself is lawful—composition is associative and unital, never commutative, and both composites exist as perfectly good morphisms. The failure is elsewhere: under concurrency, nothing in the model determines *which* composite occurs. The system's behavior isn't a single well-defined morphism—it depends on extra-categorical information (timing).
...
Products, coproducts, limits, colimits—these aren't just abstract constructions. They're the mathematically optimal solutions to specific problems. An API gateway aspires to be a product—and the product's universal property tells you exactly what an ideal gateway must satisfy.
...
[^4]: The **Yoneda lemma** states that for any functor $F: \mathcal{C}^{op} \to \text{Set}$ and object $A$, there's a natural bijection between natural transformations $\text{Hom}(-, A) \Rightarrow F$ and elements of $F(A)$. The practical intuition: an object is determined, up to isomorphism, by the morphisms into it from all other objects. You don't need to "look inside" an object—its external relationships tell you everything. Named after mathematician Nobuo Yoneda.
