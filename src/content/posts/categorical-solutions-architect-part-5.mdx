...
Consumer functor: F(A) = A → void (handlers)

// F reverses the arrow: F(getUser): F(User) → F(Id)
F(getUser) = (userHandler: User → void) => (id) => userHandler(getUser(id))
// A User-handler becomes an Id-handler — fetch, then handle
```
...
GET /users/123 → User(123)  // Reading changes nothing

// Target: reads write back a migration flag on access
GET /users/123 → stores { ...User(123), migrated: true }, then returns it

// F(id) ≠ id — "do nothing" now mutates the stored item
```
...
const process = async (data: Input): Promise<Output> => {
  const user = await fetch('/users/' + data.userId).then(r => r.json());
  const res = await fetch('/orders', {
    method: 'POST',
    body: JSON.stringify({ user, items: data.items }),
  });
  return res.json();
};
...
const withRetry = (maxRetries: number) =>
  <A, B>(call: (a: A) => Promise<B>): ((a: A) => Promise<B>) =>
  async (a: A) => {
...
const withCircuitBreaker = (config: CircuitBreakerConfig) =>
  <A, B>(call: (a: A) => Promise<B>): ((a: A) => Promise<B>) => {
...
Each layer preserves the shape of a service call. But an honest caveat: strictly, `withRetry` and `withCircuitBreaker` are not endofunctors — they're morphism decorators, transformations on hom-sets. They preserve types, not composition: retrying $g \circ f$ as a whole is not composing per-step retries, and one breaker around a pipeline is not a breaker per step — $F(g \circ f) \neq F(g) \circ F(f)$. The genuine endofunctor here is `Option` with `map`, above.
...
Injective on each hom-set: for parallel morphisms $f, g: A \to B$, if $F(f) = F(g)$, then $f = g$.
...
Surjective on each hom-set: for each pair $A, B$, every morphism $F(A) \to F(B)$ between images comes from some $f: A \to B$ in the source.
...
With unmapped morphisms, this isn't a total functor at all—it's a **partial functor**, a significant structural change.
...
[^1]: The word **functor** was borrowed by Saunders Mac Lane from Rudolf Carnap's *Logical Syntax of Language* — a debt credited in the 1945 Eilenberg–Mac Lane paper that introduced functors along with the definition of category itself. The root is the Latin verb *fungī*, "to perform" or "to execute" (unrelated to mushrooms, despite the spelling). In category theory, a functor "performs" a translation between categories. Functors formalize the notion of "structure-preserving map" — they're homomorphisms between categories, just as group homomorphisms preserve group structure and ring homomorphisms preserve ring structure.
...
[^8]: A **full functor** is surjective (onto) on hom-sets: for any two objects $A, B$, every morphism $g: F(A) \to F(B)$ in the target comes from some morphism $f: A \to B$ in the source. This means the target doesn't introduce new relationships between migrated objects — everything you can do between them in the target, you could already do in the source. A full functor "covers" all the structure. In migration terms: the new system doesn't have capabilities the old one lacked.
